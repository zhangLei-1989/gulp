...
var plumber = require('gulp-plumber');  // 处理错误异常插件
//======= gulp dev 开发环境下 ===============
function dev() {
    /** 
     * HTML处理 
    ...
    gulp.task('sass:dev', function () {
        return gulp.src(Config.sass.src).pipe(plumber()).pipe(sass()).on('error', sass.logError).pipe(gulp.dest(Config.sass.distDev)).pipe(reload({
            stream: true
        }));
    });
    /** 
